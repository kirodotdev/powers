---
name: "kiro-support"
displayName: "Kiro Support Power"
...
3. **Access probe** — run `describe_support_cases` once. Success → proceed. `AccessDenied` or
   `SubscriptionRequiredException` → [setup](steering/setup.md). Server won't start → fall back to `aws-mcp`.
...
in [filing-and-tracking-cases](steering/filing-and-tracking-cases.md#toolkit-fallback-cli).
...
## Steering — load on intent

Load only the file that matches the current intent; don't read them all up front.

| Intent | File |
| --- | --- |
| Filing, tracking, replying, resolving a case; tool names, CLI equivalents, case template | [filing-and-tracking-cases](steering/filing-and-tracking-cases.md) |
| Backend selection, onboarding, IAM/permissions, troubleshooting failures | [setup](steering/setup.md) |
| Capturing diagnostics, secrets reminder, attaching logs | [case-safety-and-diagnostics](steering/case-safety-and-diagnostics.md) |
...
The steering files and `POWER.md` are original content authored by the power author and are distributed as part of this power.

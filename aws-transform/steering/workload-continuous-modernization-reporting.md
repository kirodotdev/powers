...
Generate a single self-contained HTML report that walks through everything continuous modernization has done in this account: sources connected, repos discovered, analyses run, findings produced, remediations launched (with PR URLs). Claude assembles the HTML inline from the data it gathered and opens it in the browser.

...
This steering file guides the generation of microservice specifications from a completed DDD bounded context analysis produced in Phase 2.
...
4. **`spec/<FunctionName>/discovery/programs.yaml`** — Read to identify legacy program names (e.g., COACTUPC, COACCT01) that must be referenced in use cases.
5. **`spec/<FunctionName>/discovery/batch-jobs.yaml`** — Read to identify batch use cases that must be included (store initialization, data extraction, provisioning).

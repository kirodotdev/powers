...
After adding a source, run `atx ct discovery scan --source <name>` to discover repos. See [continuous-modernization-discovery](workload-continuous-modernization-discovery.md). Local sources also require `--path` at scan time.

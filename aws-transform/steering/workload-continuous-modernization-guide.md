...
- This IS the answer when a user asks for a remote/AWS run with no infrastructure — see [continuous-modernization-aws-managed-execution.md](workload-continuous-modernization-aws-managed-execution.md). (It is a real, shipped mode — not only `ec2`/`batch`.)
...
This guide handles continuous modernization onboarding only. For routing across Custom vs. continuous modernization (named transforms, prior findings, edge cases), see [continuous modernization routing](workload-continuous-modernization-routing.md). Do not duplicate routing logic here.

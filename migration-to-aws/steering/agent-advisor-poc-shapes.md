...
runtime. Loaded by `agent-advisor-poc.md` Step 3 (non-AgentCore verdicts) and by its
...
Content locked by `awslabs/startups:migrate/plugins/migration-to-aws/skills/agent-advisor/scripts/test_poc_shapes.py` — changing a whitelist, auth
...
- Mode A generates files only — nothing is executed. Mode B follows agent-advisor-poc.md
  Step 4's safety contract unchanged.

## agentcore

Unchanged — agent-advisor-poc.md sections 3-H (Harness) / 3-F (plan-backed framework) /
...
layout. See agent-advisor-poc.md Step 3 UNIT DISPATCH as the authority.

## Temporal worker POC (used by agent-advisor-poc.md Step 3 unit dispatch for temporal_worker_poll units)

...
and from **agent-advisor-migration-plan.md Step 2** for `build_deploy` (when `handoff-summary.md` is
...
answer → the Bedrock family per `agent-advisor-model-selection.md`; no prices), and the rationale (top scoring
...
- For infrastructure/container migration (ECS/EKS/Lambda compute): hand off to the **gcp-to-aws
...
- For AI/LLM workload migration (model swap, SDK rewrite): hand off to the **llm-to-bedrock engine**
  (the llm-to-bedrock engine) — also in this same plugin, no separate install. Point

...
_input:
  ...
  - preferences.json
_knowledge:
  - { file: heroku-dyno-eb-sizing.json, _when: "inventory has a formation AND (design_constraints.compute_target.default is elastic_beanstalk OR design_constraints.compute_target.value is elastic_beanstalk OR design_constraints.compute_target is absent OR any compute_target override is elastic_beanstalk)" }
  - { file: heroku-dyno-fargate-sizing.json, _when: "inventory has a formation AND (design_constraints.compute_target.default is ecs-fargate OR design_constraints.compute_target.value is ecs-fargate OR design_constraints.compute_target.default is elastic_beanstalk OR design_constraints.compute_target.value is elastic_beanstalk OR design_constraints.compute_target is absent OR any compute_target override is ecs-fargate)" }
  - { file: heroku-eks-pod-sizing.json, _when: "inventory has a formation AND (design_constraints.compute_target.default is eks-managed or eks-or-ecs OR design_constraints.compute_target.value is eks-managed or eks-or-ecs)" }
  - { file: heroku-postgres-rds-sizing.json, _when: "inventory has a heroku-postgresql addon" }
  - { file: heroku-redis-elasticache-sizing.json, _when: "inventory has a heroku-redis addon" }
  - { file: heroku-kafka-msk-sizing.json, _when: "inventory has a heroku-kafka addon" }
  - { file: heroku-fast-path-addons.json, _when: "inventory has a non-core addon (not postgresql/redis/kafka)" }
_fragments:
  - _id: mapping-engine
    _trigger: { _always: true }
    _file: heroku-design-mapping.md
  - _id: eks-mapping
    ...
    _file: heroku-design-eks.md
_assemble:
  _file: heroku-design-assemble.md
_produces:
  - aws-design.json

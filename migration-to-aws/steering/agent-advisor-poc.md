...
_fragments:
  - _id: poc-report
    ...
    _file: agent-advisor-poc-report.md
_assemble:
  _file: agent-advisor-poc-assemble.md
_produces:
  - plan.md
...
- entry_point ∈ {`build_scratch`, `build_deploy`}, the PRIMARY unit's `effective_runtime` in
  ...
  `agent-advisor-poc-shapes.md`, and a model-less non-agent unit omits Bedrock wiring); the gate keys on the
...
- This phase is only loaded after the user confirmed **Gate 2** (asked in agent-advisor-generate.md Step 7
  or agent-advisor-migration-plan.md Step 6).
...
When a migration plan ran, its Phase C Step 3.5 (`agent-advisor-migration-plan.md`) already reconciled every
...
`agent-advisor-freshness.md` (same anti-fabrication rule). If the MCP is not called,
...
3. **What gets created** — the AWS resources THIS runtime's shape provisions (from
   `agent-advisor-poc-shapes.md`): AgentCore Runtime + confirmed services for AgentCore; the resources in the
...
4. **Staged steps** — numbered, each with a **verification** and, where relevant, a **rollback**.
   ...
   - **Hello-agent (3a–3e):** Stage 1 review `agent.py` + config → values match intent.
     ...
     against `agent-advisor-poc-shapes.md`'s create-whitelist, and Stage 2 runs the local compile/build check
   ...
   - **lambda_microvms:** Stage 3 `./deploy.sh` per the lambda_microvms shape → deployed. Stage 4
     smoke depends on the fallback state (`agent-advisor-poc-shapes.md` lambda_microvms rule): if the MicroVMs
   ...
   - Rollback — match the runtime: **AgentCore code/3-F** → `agentcore destroy` (`TODO: verify`);
     **AgentCore Harness** → no CLI destroy, delete the CDK CloudFormation stack
     ...
     `agent-advisor-poc-<run_id>` namespace; NOT `terraform destroy`), per `agent-advisor-poc-shapes.md`. Never
...
"Effective runtime" below) (AgentCore branches for agent units on agentcore; agent-advisor-poc-shapes.md
...
- **`temporal_worker_poll` units** → the "Temporal worker POC" shape in
  `agent-advisor-poc-shapes.md`, whose BASE shape follows the unit's
...
2. **`poc/deploy.sh`** — same guardrails as 3d (cost warning + typed `deploy` confirmation),
   ...
   deploy` with an `agent-advisor-runtime-agentcore.json` project config) — NOT via the Python starter-toolkit's
...
- `README.md`: per 3e, PLUS, prominently: (a) "This is the migrated version of YOUR app
  ...
  quality eval), run the llm-to-bedrock engine" — the POC does not replace it;
...
Load `agent-advisor-poc-report.md` and follow it to produce

...
> Loaded by `agent-advisor-generate.md` after Steps 3–5 complete (diagram written, recommendation.md
...
3-card form from agent-advisor-report-help-banner.md — **but ONLY when `agent-advisor-report-help-banner.md`'s
...
`agent-advisor-report-help-banner.md` — never hardcode it here.
...
| `TEMPORAL_COST_TABLE`         | from agent-advisor-estimate.md (Polling tier, Execution tier, Temporal Cloud actions, What it replaces)                                                                                                                                                                                                                                                                                                                   | []                                 |
...
<!-- SRI-pinned mermaid@10.9.3 script tag — inline it VERBATIM from the shared shell
     (agent-advisor-report-shell.md), same tag/integrity hash used by every report with a diagram. -->
...
<!-- ═══ HELP STRIP — GATED on agent-advisor-report-help-banner.md `banner_status`. It currently reads
     ...
     {{ HELP_URL }} with the single-source destination URL from agent-advisor-report-help-banner.md (do NOT
...
Output to the parent `agent-advisor-generate.md`:

```
Recommendation report written to {{ RUN_DIR }}/recommendation-report.html
```

Do NOT update `.phase-status.json` — the parent `agent-advisor-generate.md` handles phase completion.

...
> Loaded by agent-advisor-poc.md Step 4.5 after the POC files are written (Step 3) — for BOTH modes.
...
inline CSS from `agent-advisor-report-shell.md` + SRI-pinned Mermaid — no other external
dependencies. The report sits in `$RUN_DIR/poc/`, so artifact download links are relative to
that dir.

**Before writing the HTML:** load the shared shell (`agent-advisor-report-shell.md`) — inline
...
other shared components. The help CTA is GATED on `agent-advisor-report-help-banner.md`'s `banner_status`:
...
from `agent-advisor-report-help-banner.md` — never hardcode it here.
...
| `MODEL_DISPLAY`    | resolved Bedrock model PER UNIT (Step 2 of agent-advisor-poc.md) — `null`/omitted for a unit whose `model_recommendation` is null; never a single global model           |
...
after `deploy.sh`), using the same topology discipline as agent-advisor-build-diagram.md. **Render one node per
...
- For plan-backed framework POCs: annotate the original UI (e.g. "Chainlit UI — local dev
  ...
  Include an ASCII fallback in a `<details>` block, same as agent-advisor-build-diagram.md.
...
<!-- SRI-pinned mermaid@10.9.3 script tag — inline it VERBATIM from the shared shell
     (agent-advisor-report-shell.md), same tag/integrity hash. -->
{{ SHARED_SHELL_MERMAID_TAG from agent-advisor-report-shell.md }}
<style>
  /* ── Shared chrome ── load agent-advisor-report-shell.md and inline its CSS block
  ...
  {{ SHARED_SHELL_CSS from agent-advisor-report-shell.md }}
...
<!-- ═══ HELP STRIP — GATED on agent-advisor-report-help-banner.md `banner_status`. It currently reads
     ...
     single-source destination URL from agent-advisor-report-help-banner.md (do NOT hardcode the URL here):
...
<div class="doc-foot">
  ...
  <code>the llm-to-bedrock engine</code>. Generated {{ RUN_DATE }}.
</div>
...
Return to agent-advisor-poc.md. Do NOT update `.phase-status.json` — agent-advisor-poc.md Step 6 handles phase state.

...
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| **About to author `terraform/`** (before writing)           | Part 1 → [`security-posture-rules.md`](security-posture-rules.md)                   | The "what to emit" AWS authoring rules (gate-enforced + authoring-only + compliance-conditional). |
| **`terraform/` written, ready to validate** (after writing) | Part 2 → [`terraform-validation.md`](terraform-validation.md) + run the gate script | The `fmt → init → validate → policy` protocol and the read-only verdict.                          |
...
`security-posture-rules.md` § _Caller-context signals_.
...
[`security-posture-rules.md`](security-posture-rules.md).
...
`$STEERING` (`$STEERING/tf-validate-terraform-policy.py`), the same convention the
...
[`terraform-validation.md`](terraform-validation.md). That protocol is
...
`awslabs/startups:migrate/plugins/migration-to-aws/skills/tf-best-practices/fixtures/terraform-policy/`.
...
```bash
uv run $STEERING/tf-validate-terraform-policy.py <directory containing .tf files>
```

...
| State shape       | `phase-status.schema.json` | its own state-file shape, documented in SKILL.md, when its state carries skill-specific keys. The update protocol and validation below still apply.                                                                                                                                                                     |
...
2. **Determine the current phase (deterministic):**
   - **Deferred-advance sidebar resume (mandatory):** If `current_phase` is
     ...
     load `<sidebar>.md` if `"in_progress"`.
...
4. **Load the phase orchestrator.** A phase's orchestrator file is, by convention,
   `<phase>.md`. Load it in full and read its
...
| `_knowledge`        | the reference/data files the phase consults (`*.json` sizing/mapping/pricing tables). Each entry is `{ file, _when? }`; each `file` must resolve on disk. Load a knowledge file ONLY when its `_when` holds — see § `_knowledge`.                                                                                                                                                                                                                                                                                                     |
...
`*.json` lookup tables it consults (sizing, mapping, pricing). Each
...
```
Worker contract (Read this FIRST and follow it): <$STEERING>/generic-phase-worker-rw.md
Skill: <the engine name, e.g. heroku-to-aws>
Skill root: <$STEERING, absolute — every reference file is flat in this one directory>
...
4. Write `.phase-status.json` per the schema
   `phase-status.schema.json`. Seed `phases` with ONE entry per

...
See `llm-openai-to-bedrock.md` § `temperature-range-mismatch` § Code templates for the full set: `range_narrowed_1` (Cap UI), `range_narrowed_2` (Linear rescale), `range_narrowed_3` (Keep + note), `range_narrowed_4` (Fail loud).
...
Use the `parameter_removed` set from the skill (see `llm-behavior-delta-detection.md` § Option set: parameter_removed). Substitute the Gemini SDK call site:
...
Same conventions as `llm-openai-to-bedrock.md`. Update the "last verified" date in this file's header.

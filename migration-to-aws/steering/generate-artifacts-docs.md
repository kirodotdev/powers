...
**Graviton Migration Notes** — Include when any compute resource in `aws-design.json` has `graviton.target_architecture == "arm64"` (see `graviton.md`). Add a short subsection in Section 4 listing:
...
Bullet list from design and generation artifacts: Compute, Database, Storage, and AI/ML (if applicable) with GCP service, AWS service, and rationale. For each GCP→AWS mapping, add how it was chosen using `design-ref-fast-path.md` → **User-facing vocabulary**: **Standard pairing**, **Tailored to your setup**, or **Estimated from billing only** (from the design artifact’s `confidence` field). For any service with `human_expertise_required: true`, append: "(Specialist guidance recommended — contact your AWS account team)".
...
- **Key decisions**: Extract from design artifact `rationale` fields and map `confidence` to user-facing labels per `design-ref-fast-path.md` → **User-facing vocabulary**

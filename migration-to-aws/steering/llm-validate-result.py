...
1. Phase-result validation:
     llm-validate-result.py --schema {analysis,ingestion,eval,rewrite,delta-decisions} <file>
...
2. Run-context comparison:
     llm-validate-result.py --check-run-context <saved.json> --current <current.json>
...
def validate_phase(schema_name: str, file_path: str) -> int:
    ...
    if err:
        ...
        return 2
    schema, err = load_json(str(SCHEMAS_DIR / f"llm-{schema_name}.schema.json"))
    if err:

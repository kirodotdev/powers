...
_of_phase: intake
_reads:
  - entry-point + background answers (collected inline in agent-advisor-intake.md Steps 2–4)
_produces:
  - .phase-status.json
---
...
> context inline within `agent-advisor-intake.md`, then writes the run's `.phase-status.json`
...
> on the phase) are the phase's completion gate. See `agent-advisor-intake.md` § Step 5 for the

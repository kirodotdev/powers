...
> are owned by the assembler (`heroku-estimate-assemble.md`).
...
Execute `pricing-mode.md` (the canonical
Step 0, vendored from `pricing-mode.md` and kept
...
2. **Live-captured prices + dyno cache** — If no billing data AND at least one
   ...
   - **Dynos:** the API does not price formations; look up each formation's
     `dyno_type` in `heroku-pricing-cache.md` (±5% published
     flat rates) × `quantity`
...
3. **Heroku pricing cache** — If no billing data and no live-captured prices, Load `heroku-pricing-cache.md` and derive costs from discovered resources:
...
For each service in `aws-design.json → services[]`, calculate monthly cost by applying the formula from the Per-Service Calculation Formulas table below, looking up its rates from `aws-infra-pricing.json`. Track `pricing_source` per service. (`hours_per_month` = 730, from `_meta`.)
...
(All `cloudwatch.*` rates are in `aws-infra-pricing.json`.)
...
The savings ranges + applicability come from [`heroku-estimate-defaults.json`](heroku-estimate-defaults.json) → `optimization_savings_ranges` (keys: `compute_savings_plans`, `database_savings_plans`, `rds_reserved_instances`, `s3_intelligent_tiering`, `fargate_spot`, `ec2_spot`; each carries `savings_percent` / `target_services` / `timing`). Include ONLY opportunities relevant to the designed architecture (per each entry's `target_services` / `timing`).
...
Load the tier thresholds declared in `_knowledge` (`complexity-tiers.json`). Classify using these inputs from the current artifacts:
...
When Parts 1–8 are complete, control passes to the assembler (`heroku-estimate-assemble.md`) to
...
Only use these recipes when a service is NOT in `aws-infra-pricing.json` and MCP is available. Do NOT call `get_pricing_service_codes` or `get_pricing_service_attributes` — go directly to `get_pricing`.

...
The phase assembler (`heroku-discover-assemble.md`) owns the inventory's STRUCTURE (which
...
After generating the resource entries and conflict records, the parent `heroku-discover.md` handles merging into the final inventory and updating phase status — do NOT update `.phase-status.json` from this sub-file.

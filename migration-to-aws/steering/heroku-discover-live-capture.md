...
> two things the dispatched `rw` worker cannot do. `heroku-discover.md` § Orientation says
...
> `manifest.json` index. The `live` FRAGMENT (`heroku-discover-live.md`) parses those files
...
- **[B]** → do not run any Heroku command. Return to `heroku-discover.md` and record that
...
## Step 5: Return to `heroku-discover.md`

Tell the user in one line how many apps were captured and whether any captures
failed or were skipped, then continue the phase per `heroku-discover.md` (the dispatched

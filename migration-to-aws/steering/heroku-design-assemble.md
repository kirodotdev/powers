...
> **Assembler unit.** Runs after the mapping fragments (`heroku-design-mapping.md`, and
> `heroku-design-eks.md` when EKS is selected) have populated the in-memory design object.
...
> `heroku-workshop-refresh.md`, stop after writing `aws-design.json` (and soft
> validation). Skip this Step 8 and do **not** emit `HANDOFF_OK` — see
> `heroku-workshop-refresh.md` § Inner runs.
...
Format: "Design phase complete. [artifact summaries] Next required step: Phase 4 — Estimate. Load `heroku-estimate.md` now."

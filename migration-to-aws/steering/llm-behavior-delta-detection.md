...
`resolution_chosen` is a typed enum: `"{option_set_id}_{1-indexed_option_number}"` for ux_choice and fallback paths, or `"impl_path_default"` for impl_path defaults. The `behavior_deltas` item schema inside `llm-analysis.schema.json` is the authoritative field contract (the recording shape is `llm-delta-decisions.schema.json`).

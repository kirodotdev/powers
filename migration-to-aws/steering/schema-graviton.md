...
> Canonical schemas for Graviton/ARM64 fields. Loaded by `discover-app-code.md` and `discover-iac.md` (to write `graviton_profile`), `design-ref-compute.md` (to write the per-service `graviton` block), and `estimate-infra.md` (to write `architecture_comparison`). See `graviton.md` for behavior.
...
**Report consistency:** when the migration report renders Graviton savings, the figures MUST equal these fields (no recomputation in the report layer). This is a **manual self-check** today; PR #78's post-write report validator is structural/readability only and does not audit dollar figures. An automated numeric assertion is a tracked follow-up (see `graviton.md` → "Report rendering").

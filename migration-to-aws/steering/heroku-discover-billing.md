...
> If no billing files are found, exits cleanly with no output. The parent `heroku-discover.md` handles graceful degradation.
...
Produce the `billing_profile` section for inclusion in `heroku-resource-inventory.json`. The parent `heroku-discover.md` Step 3 merges this into the final inventory.
...
After generating the billing profile output, the parent `heroku-discover.md` handles merging it into the inventory and updating phase status — do NOT update `.phase-status.json` here.

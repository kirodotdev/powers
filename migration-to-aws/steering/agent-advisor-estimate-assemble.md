...
_of_phase: estimate
_reads:
  - design.json + cached rate anchors (combined inline in agent-advisor-estimate.md)
_produces:
  - estimate.json
---
...
> monthly magnitude, and writes `estimate.json` inline within `agent-advisor-estimate.md`
...
> phase) are the phase's completion gate. See `agent-advisor-estimate.md` § Step 4 for the

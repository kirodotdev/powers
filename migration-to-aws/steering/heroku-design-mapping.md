...
> selects EKS, formation mapping is handled by `heroku-design-eks.md` instead of the
> EB/Fargate branches below. The final artifact write, output checks, handoff gate, and
> phase-status update are owned by the assembler (`heroku-design-assemble.md`).
...
2. When `"arm64"`:
   - **Elastic Beanstalk**: use `ec2_instance_type_arm64` from the matched `heroku-dyno-eb-sizing.json` row when present; else use `ec2_instance_type` and append warning `architecture_fallback_x86` naming the formation.
   - **Fargate**: set `aws_config.cpu_architecture` to `"ARM64"` (task size from `heroku-dyno-fargate-sizing.json` unchanged — Fargate ARM is a runtime arch flag). Prefer Estimate rates `fargate.per_vcpu_hour_arm64` / `per_gb_mem_hour_arm64` when present.
   - **EKS**: use `node_type_arm64` from `heroku-eks-pod-sizing.json` when present; else `node_type` + `architecture_fallback_x86` warning.
...
- If `formation_compute_target` is absent or `"elastic_beanstalk"`: use `heroku-dyno-eb-sizing.json` and follow the Elastic Beanstalk mapping below, except persistent non-web formations with `quantity > 1` MUST resolve to Fargate for that formation to preserve horizontal worker capacity.
- If `formation_compute_target` is `"ecs-fargate"`: use `heroku-dyno-fargate-sizing.json` and follow the Fargate mapping below.
- If `global_compute_target` is `"eks-managed"` or `"eks-or-ecs"`: use `heroku-eks-pod-sizing.json` and load `heroku-design-eks.md`. Follow the EKS branch logic in `heroku-design-eks.md` for ALL formations. Skip the EB/Fargate mapping below. Per-formation EKS overrides are not supported in v1; EKS remains a global compute target.

#### EKS Branch

When `global_compute_target` is `"eks-managed"` or `"eks-or-ecs"`, load and follow `heroku-design-eks.md`. That file contains the complete EKS mapping logic. ALL formations are mapped to EKS. Return here after EKS mapping is complete (skip the EB/Fargate logic below).
...
3. **Dyno type lookup**: Match `config.dyno_type` (case-insensitive) against the `heroku-dyno-eb-sizing.json` knowledge (`rows.<dyno_type>`).
...
3. **Dyno type lookup**: Match `config.dyno_type` (case-insensitive) against the `heroku-dyno-fargate-sizing.json` knowledge (`rows.<dyno_type>`).
...
**Prerequisites**: the `heroku-postgres-rds-sizing.json` knowledge (loaded per the phase's `_knowledge` `_when`).
...
**Prerequisites**: the `heroku-redis-elasticache-sizing.json` knowledge (loaded per the phase's `_knowledge` `_when`).
...
**Prerequisites**: the `heroku-kafka-msk-sizing.json` knowledge (loaded per the phase's `_knowledge` `_when`).
...
**Prerequisites**: the `heroku-fast-path-addons.json` knowledge (loaded per the phase's `_knowledge` `_when`).
...
to the assembler (`heroku-design-assemble.md`) to write `aws-design.json`, run the output checks +

...
_fragments:
  - _id: terraform
    ...
    _file: heroku-discover-terraform.md
  - _id: live
    ...
    _file: heroku-discover-live.md
  - _id: billing
    _trigger: { _glob: "**/*{billing,invoice}*.{csv,json}" }
    _file: heroku-discover-billing.md
_assemble:
  _file: heroku-discover-assemble.md
_produces:
...
1. **Capture** (`heroku-discover-live-capture.md`) — runs in the MAIN window, after
...
2. **Parse** (`heroku-discover-live.md`, the `live` fragment) — runs in the worker with the
...
the primary source — load `heroku-discover-live-capture.md` — instead of failing the check.
...
never silently resolved — see `heroku-discover-assemble.md` § Merge & Drift Rules.
...
Format: "Discover phase complete. [artifact summaries] Next required step: Phase 2 — Clarify. Load `heroku-clarify.md` now. Do not load Design, Estimate, or Generate until Clarify completes and `.phase-status.json` marks `phases.clarify` as `completed`."

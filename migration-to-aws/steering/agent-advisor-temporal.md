...
`awslabs/startups:migrate/plugins/migration-to-aws/skills/agent-advisor/scripts/test_temporal_decision_refs.py` — if you change the Tier 1 rules or
...
| Agent-session (LLM loop + tools + memory, tens of minutes) | stateful session, isolation, independent scaling | **run `agent-advisor-scoring.py`** via the adapter table below (§ Tier 2 adapter — agent-session Activity classes); typical winner AgentCore Runtime (≤8h) — DAF pattern; community reference github.com/koukish/async-agent-architecture-sample (not official) |
...
For agent-session Activities, build the agent-advisor-scoring.py input via this adapter (unlisted dimensions stay `"unknown"` — the engine's DEFAULTS apply):

| agent-advisor-scoring.py dimension                                    | Fill from Temporal context                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
and label the output pre-release regardless (see agent-advisor-freshness.md, Temporal section).
